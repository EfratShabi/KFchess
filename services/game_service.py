...
class GameService:
    ...
    def _select_piece(self, pos):
        if not self.board.is_empty(pos.row, pos.col):
            self.selected_piece = pos.as_tuple()


    def _try_move(self, pos):
        prev_tuple = self.selected_piece
        piece_str = self.board.get_piece_str(*prev_tuple)
        ...
        if self.rules.is_valid_move(self.board.grid, piece_str, prev_tuple, pos.as_tuple()):
            distance = self.rules.calc_distance(prev_tuple, pos.as_tuple())
            self.state.register_move(prev_tuple, pos.as_tuple(), piece_str, distance)
        self.selected_piece = None
