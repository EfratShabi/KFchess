...
def test_movement_not_due_before_arrival():
    m = Movement('wR', (0, 0), (0, 2), arrival_time=2000)
...
def test_movement_stores_piece():
    m = Movement('bK', (1, 1), (2, 2), arrival_time=1000)
    assert m.piece == 'bK'

def test_movement_stores_start_end():
    m = Movement('wN', (0, 0), (2, 1), arrival_time=500)
    assert m.start == (0, 0)
    assert m.end == (2, 1)
